...
// .wrangler/tmp/bundle-ogSCTC/strip-cf-connecting-ip-header.js
    ...
    const content = `
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-indigo-500/50 transition-all duration-300">
          ...
          <p class="text-xs text-gray-500 mt-2">Murid & Staff</p>
        </div>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-2xl p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div class="bg-gray-950/50 border border-gray-800 rounded-xl p-4">
            <p class="text-sm font-semibold text-gray-300 mb-2">\u{1F4CB} Alur Kerja:</p>
            <ol class="text-xs text-gray-500 space-y-1 list-decimal list-inside">
              <li>Input data responden & kriteria</li>
...
app.get("/respondents", async (c) => {
  ...
  try {
    ...
    const params = [];
    if (typeFilter) {
      query += " WHERE consumer_type = ?";
      countQuery += " WHERE consumer_type = ?";
      params.push(typeFilter);
    }
    query += " ORDER BY CAST(SUBSTR(id, 2) AS INTEGER) ASC LIMIT ? OFFSET ?";
    const countResult = await c.env.DB.prepare(countQuery).bind(...typeFilter ? [typeFilter] : []).first();
    ...
    const rows = results.length > 0 ? results.map((row) => {
      ...
      return `
          ...
          `;
    }).join("") : '<tr><td colspan="3" class="p-8 text-center text-gray-500">Tidak ada data responden</td></tr>';
    const filterActive = /* @__PURE__ */ __name((val) => typeFilter === val ? "selected" : "", "filterActive");
    const pagination = totalPages > 1 ? `
      <div class="flex justify-between items-center bg-gray-900 border border-gray-800 rounded-xl p-4 mt-6">
        <span class="text-sm text-gray-400">
          Menampilkan ${offset + 1} - ${Math.min(offset + limit, total)} dari ${total} responden
        </span>
        <div class="flex gap-2">
          ${page > 1 ? `<a href="/respondents?page=${page - 1}${typeFilter ? `&type=${typeFilter}` : ""}" class="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition-colors border border-gray-700">\u2190 Sebelumnya</a>` : ""}
          <span class="px-4 py-2 text-sm text-gray-400">Halaman ${page} / ${totalPages}</span>
          ${page < totalPages ? `<a href="/respondents?page=${page + 1}${typeFilter ? `&type=${typeFilter}` : ""}" class="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition-colors border border-gray-700">Selanjutnya \u2192</a>` : ""}
        </div>
      </div>
    ` : "";
    const content = `
      <div class="flex justify-between items-center mb-8">
        <div>
          ...
          <p class="text-gray-400 mt-1">Total: ${total} responden</p>
        </div>
        <form method="get" class="flex gap-2">
          <select name="type" class="bg-gray-900 border border-gray-700 text-white text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5">
...
app.get("/evaluate", async (c) => {
  try {
    const { results: respondents } = await c.env.DB.prepare("SELECT id, name FROM respondents ORDER BY CAST(SUBSTR(id, 2) AS INTEGER) ASC").all();
    const { results: criteria } = await c.env.DB.prepare("SELECT * FROM criteria ORDER BY id ASC").all();
    const respondentOptions = respondents.map((r) => `<option value="${r.id}">${r.id} - ${r.name}</option>`).join("");
...
app.get("/results", async (c) => {
  ...
  try {
    ...
    const totalPages = Math.ceil(total / limit);
    const query = `
      SELECT s.respondent_id, r.name, r.consumer_type, s.final_score, s.satisfaction_level
      FROM saw_results s
    ...
    const rows = results.length > 0 ? results.map((row, idx) => {
      ...
      return `
          ...
          `;
    }).join("") : '<tr><td colspan="6" class="p-8 text-center text-gray-500">Belum ada hasil perhitungan. <a href="/saw-calculate" class="text-indigo-400 underline">Jalankan SAW</a></td></tr>';
        ...
        <table class="w-full text-left text-sm">
          <thead class="bg-gray-950 border-b border-gray-800">
            <tr>
              ...
              <th class="p-4 font-semibold text-gray-400">Nama Responden</th>
...
// .wrangler/tmp/bundle-ogSCTC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ogSCTC/middleware-loader.entry.ts
